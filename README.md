...
Komponen yang **sudah diimplementasikan** untuk memenuhi Final Project:

| Komponen | Status |
|---|---|
| Batch pipeline (API → GCS → BigQuery Raw → Staging → DQ → Curated) | ✅ Implemented, terjadwal tiap 15 menit dengan mekanisme next_date otomatis |
| Streaming pipeline (API polling → Pub/Sub → Consumer → Raw → Staging → DQ → Curated) | ✅ Implemented sebagai near-real-time scheduled polling (tiap 5 menit), bukan continuous streaming |
| Data Lake (GCS) | ✅ Implemented |
| Data Warehouse (BigQuery: Raw/Staging/Curated/Dimension) | ✅ Implemented |
| Transformation (dbt staging & curated) | ✅ Implemented |
| Data Quality (monitoring layer, 5 rule) | ✅ Implemented |
| Alerting (email on task failure) | ✅ Implemented |
| Dimension pipeline (GitHub JSON → PostgreSQL → BigQuery) | ✅ Implemented, orkestrasi Airflow belum ada (lihat bagian 14) |
| Dashboard (Looker Studio) | ⏳ Kebutuhan query & desain visual sudah diaudit; dashboard belum dibangun |
