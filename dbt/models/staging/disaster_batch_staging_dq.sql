...
-- Validates disaster_batch_staging - never changes values or drops records.
-- Only failed checks are logged here (one row per record per failed rule);
-- no row means that check passed. Full table, not incremental, so this
-- always reflects current staging.

with staging as (

    select *
    from {{ ref('disaster_batch_staging') }}
...
duplicate_checks as (
    ...
    select
        ...
        concat('record_key appears ', cast(key_count as string), ' times in disaster_batch_staging') as issue_detail,
        current_timestamp() as checked_at
    from (
